      ...
      {CartItems.map((product, productIdx) => (
        <li key={product._id} className="flex py-6 sm:py-10">
          ...
          <div className="ml-4 flex flex-1 flex-col justify-between sm:ml-6">
            {/* Eco Impact Badge */}
            <div className="mb-2">
              <span className="inline-flex items-center rounded-full bg-green-100 dark:bg-green-900 px-3 py-1 text-xs font-semibold text-green-700 dark:text-green-300">
                🌿 {getEcoImpact(product)}kg CO₂ saved
              </span>
            </div>
            <div className="relative justify-between pr-9 sm:flex sm:gap-x-6 sm:pr-0">
              <div>
                ...
                <p className="mt-1 text-sm font-medium">{formatPrice(product.price, product.currency)}</p>
              </div>
